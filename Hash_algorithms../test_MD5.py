...
def md5(message):
    ...
    #инициализация 4 переменных 
    init_abcd = [0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476]

    #определение 4 функций для 4 раундов 
    ...
    #64-битное представление длины данных (если длина > 2^64-1)
    #то записываем только младшие биты
    orig_len_in_bits = (8 * len(message)) & 0xffffffffffffffff
    
    #дописываем единичный бит в конец потока
    message.append(0x80)
    
    #выравнивание потока (сравним с 448 по модулю 512)
    while len(message)%64 != 56:
        message.append(0)
    
    #записываем сначала младшие байты в конец потока данных
    message += orig_len_in_bits.to_bytes(8, byteorder='little')
 
    hash_pieces = init_abcd[:]
 
    #вычисление в цикле: 
    #производим смещение chunk на 64 байта(512 бит)
